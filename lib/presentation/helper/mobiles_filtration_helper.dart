...
class MobilesFiltrationHelper{
  ...
  List<MobileWithThemeModel> filterSearchResults({required List<MobileWithThemeModel> mobilesWithTheme, required String searchText}) {
  ...
  List<MobileWithThemeModel> getMobilesArrangementWithTheme({required List<MobileModel> mobiles}){
  ...
  List<MobileWithThemeModel> getMobilesWithTheme({required List<MobileModel> mobiles}){
  ...
  MobileWithThemeModel getMobileWithTheme(MobileModel mobile){
    switch (mobile.brandName.toLowerCase()) {
      case 'apple':
        return MobileWithThemeModel(mobileModel: mobile,
  ...
  List<MobileWithThemeModel> getBrandMobilesWithTheme(
    ...
    List<MobileWithThemeModel> list = [];
    List<MobileModel> brandMobiles = getBrandMobiles(mobiles: mobiles, brandName: brandName);
  ...
  List<MobileModel> getBrandMobiles({required List<MobileModel> mobiles, required String brandName}){
  ...
  Either<bool, MobileWithThemeModel> isScreenCompatible({required MobileModel requiredMobile, required MobileModel comingMobile}){
    return _isCompatible(requiredMobile: requiredMobile, comingMobile: comingMobile, biggestDifferenceBetweenScreenSizes: biggestPossibleDifferenceBetweenDisplaysInScreens);
  }

  Either<bool, MobileWithThemeModel> isGlassCompatible({required MobileModel requiredMobile, required MobileModel comingMobile}){
    return _isCompatible(requiredMobile: requiredMobile, comingMobile: comingMobile, biggestDifferenceBetweenScreenSizes: biggestPossibleDifferenceBetweenDisplaysInGlass);
  }

  Either<bool, MobileWithThemeModel> _isCompatible({required MobileModel requiredMobile, required MobileModel comingMobile, required double biggestDifferenceBetweenScreenSizes}){
    ...
    if (requiredBrand.toLowerCase() == appleText.toLowerCase()){
      ...
      double differenceBetweenTwoScreenSizes = double.parse((requiredDisplaySize - comingDisplaySize).abs().toStringAsFixed(2));

      if (requiredMobileName != comingMobileName
          && differenceBetweenTwoScreenSizes <= biggestDifferenceBetweenScreenSizes){
        return Right(getMobileWithTheme(requiredMobile));
      }
    }
  ...
  List<MobileWithThemeModel> getScreensCompatibilities({required List<MobileModel> mobiles, required MobileModel mobile}){
    return getCompatibilities(mobiles: mobiles, mobile: mobile, biggestDifferenceBetweenScreenSizes: biggestPossibleDifferenceBetweenDisplaysInScreens);
  }

  List<MobileWithThemeModel> getCoversCompatibilities ({required List<MobileModel> mobiles, required MobileModel mobile}){
  ...
  List<MobileWithThemeModel> getGlassCompatibilities({required List<MobileModel> mobiles, required MobileModel mobile}){
    return getCompatibilities(mobiles: mobiles, mobile: mobile, biggestDifferenceBetweenScreenSizes: biggestPossibleDifferenceBetweenDisplaysInGlass);
  }


  List<MobileWithThemeModel> getCompatibilities({required List<MobileModel> mobiles, required MobileModel mobile, required double biggestDifferenceBetweenScreenSizes}){
    ...
    if (requiredBrandName == appleText.toLowerCase()){
      for (int mobileModelIndex = 0; mobiles.length > mobileModelIndex; mobileModelIndex++){
    ...
    // else required mobile is from other brand other than Apple
      ...
      for (int brandNameIndex = 0; brandsNamesTextsList.length > brandNameIndex; brandNameIndex++){
